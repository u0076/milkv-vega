...
static int spinand_read_id_op(struct spinand_device *spinand, u8 *buf)
{
	...
	struct spi_mem_op op1 = SPINAND_READID_OP_WINBOND(1,spinand->scratchbuf,SPINAND_MAX_ID_LEN);
	int ret,i;

	ret = spi_mem_exec_op(spinand->slave, &op1);
	if (!ret)	
		memcpy(buf, spinand->scratchbuf, SPINAND_MAX_ID_LEN);
	
	if(0xd1 == buf[1] && 0xc8 == buf[2])
	{
		buf[1] = 0xc8;
		buf[2] = 0xd1;
...
#else
///	ret = add_mtd_device(mtd);//added by pfliu

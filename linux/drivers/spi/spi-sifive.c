...
#define SIFIVE_SPI_DEFAULT_DEPTH         6   //added by pfliu
...
static void sifive_spi_init(struct sifive_spi *spi)
{
	...
	sifive_spi_write(spi, SIFIVE_SPI_REG_DELAY0, SIFIVE_SPI_DELAY0_CSSCK(1) 
		| SIFIVE_SPI_DELAY0_SCKCS(1));
	sifive_spi_write(spi, SIFIVE_SPI_REG_DELAY1, SIFIVE_SPI_DELAY1_INTERCS(1) 
...
//	printk("# spi @0x%x device mode 0x%x,cs_inactive 0x%x,chip_select 0x%x\n",spi->regs,
//		device->mode,spi->cs_inactive,device->chip_select);		
...
/*
	tmp = sifive_spi_read(spi, SIFIVE_SPI_REG_CSID);
	printk("# SIFIVE_SPI_REG_CSID:0x%x\n",tmp);	
*/
...
/*
	tmp = sifive_spi_read(spi, SIFIVE_SPI_REG_CSMODE);
	printk("#sifive_spi_set_cs SIFIVE_SPI_REG_CSMODE:0x%x\n",tmp);	
*/
}
...
/*
	tmp = sifive_spi_read(spi, SIFIVE_SPI_REG_FMT);
	printk("#sifive_spi_prep_transfer SIFIVE_SPI_REG_FMT:0x%x\n",tmp);
*/
...
{
//	printk("#line=%d  function=%5s,bit=0x%x,poll=0x%x\n",__LINE__,__FUNCTION__,bit,poll);
...
static int sifive_spi_probe(struct platform_device *pdev)
{
	...
	/* Calculate and program the clock rate */
	//unsigned long spi_clock = clk_get_rate(spi->clk);
	//printk("spi bus clock = %ld\n", spi_clock);

	struct device_node *flash_node = of_get_next_child(pdev->dev.of_node, NULL);
	if (flash_node == NULL)
		printk("fail to get flash node\n");

	//printk("flash name: %s",flash_node->name);
	
	u32 flash_clk;
	of_property_read_u32(flash_node, "spi-max-frequency", &flash_clk);
	//printk("flash clock = %d\n", flash_clk);
	u32 tmp = (DIV_ROUND_UP(clk_get_rate(spi->clk) >> 1, flash_clk) - 1) & SIFIVE_SPI_SCKDIV_DIV_MASK;
	//printk("linux spi sck div = %d\n", tmp);
	sifive_spi_write(spi, SIFIVE_SPI_REG_SCKDIV, tmp);	
